...
class laporan extends CI_Controller
{
    ...
    function index()
    {
        ...
        $query="SELECT a.*,b.*,c.* FROM rekam_langgar as a, dt_siswa as b, j_pelanggaran as c where a.nisn_pelanggar=b.nisn and a.pelanggaran=c.id_langgar 
                and a.tgl_pelanggaran BETWEEN '".$data['tanggal1']."' and '".$data['tanggal2']."'";
        $data['transaksi']=  $this->db->query($query)->result();
        $this->template->load('template', 'laporan/view',$data);
    }
    
    function laporan()
    {
        $data['title']=  'Laporan Pencatatan';
    ...
    function cetak()
    {
        ...
        $this->load->view('laporan/cetaklap',$data);
    }
    ...
    function laporanpembayaran()
    {
        ...
        $tanggal2=  $this->uri->segment(4);
               $query="SELECT a.nama_santri,a.santri_id,a.kelas_id,a.jk,b.nama_peraturan,c.nama_level,d.nama_pelanggaran,d.sanksi, e.tanggal_pelanggaran, f.nama_kelas, g.nama_madrasah
                FROM pencatatan_pelanggaran as e, santri as a, peraturan as b, level as c, pelanggaran as d, akademik_kelas as f, akademik_madrasah as g
                WHERE e.santri_id=a.santri_id and e.pelanggaran_id=d.pelanggaran_id and d.peraturan_id=b.peraturan_id and d.level_id=c.level_id and a.kelas_id = f.kelas_id and f.madrasah_id=g.madrasah_id
                and e.tanggal_pelanggaran BETWEEN '".$tanggal1."' and '".$tanggal2."'";
        $data['transaksi']=  $this->db->query($query)->result();
        $this->load->view('laporan/laporanpelanggaran',$data);
    }
